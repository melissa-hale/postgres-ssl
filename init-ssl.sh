...
if [ ! -f "$SSL_DIR/server.key" ] || [ ! -f "$SSL_DIR/server.crt" ] || [ ! -f 
"$SSL_DIR/root.crt" ]; then
    # Generate Root CA
    openssl req \
        -new -x509 -days "${SSL_CERT_DAYS:-820}" \
        -nodes -text -out "$SSL_DIR/root.crt" \
        -keyout "$SSL_DIR/root.key" -subj "/CN=root-ca" \
        -extensions v3_ca -config "$SSL_CONF_DIR/ssl-extensions.conf"
    ...
    openssl x509 \
        ...
        -extfile "$SSL_CONF_DIR/ssl-extensions.conf" -extensions v3_server_cert \
        -out "$SSL_DIR/server.crt" -CA "$SSL_DIR/root.crt" \
